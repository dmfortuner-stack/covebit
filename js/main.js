...
function initBookingEngine() {
  ...
  function sendBookingConfirmationEmails(booking) {
    ...
    const emailBody = [
      ...
      (!booking.isOnCall && refTag) ? `'Please mention booking reference number (${refTag}) while making payment as remark.'` : '',
      '',
      !booking.isOnCall ? 'Payment Options:' : '',
      (!booking.isOnCall && booking.stripeUrl) ? `• Stripe (Credit / Debit Card) : ${booking.stripeUrl}` : '',
      (!booking.isOnCall && booking.paypalUrl) ? `• PayPal (Instant Checkout)    : ${booking.paypalUrl}` : '',
      '',
      '==================================================',
      'WHAT HAPPENS NEXT?',
      '==================================================',
